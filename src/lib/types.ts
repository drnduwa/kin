...
export const signupSchema = z.object({
  name: z.string().min(2, { message: "Le nom doit comporter au moins 2 caract√®res." }),
  email: z.string().email({ message: "Veuillez entrer une adresse e-mail valide." }),
  password: z.string().min(8, { message: "Le mot de passe doit comporter au moins 8 caract√®res." }),
  phone: z.string().min(9, { message: "Veuillez entrer un num√©ro de t√©l√©phone valide." }),
...
export const StrategicInsightsOutputSchema = z.object({
  ...
  trend: z.enum(['am√©lioration', 'stable', 'd√©gradation']),
});
...
export interface HourlyForecast {
  time: string;
  status: "EMBOUTEILLAGE" | "DENSE" | "MOD…R…" | "FLUIDE" | "INCONNU";
  delay: number;
}

export interface TrafficForecast {
  road: string;
  currentStatus: "EMBOUTEILLAGE" | "DENSE" | "MOD…R…" | "FLUIDE" | "INCONNU";
  currentDelay: number;
